...
// Deleting a baker isn't exposed in the UI yet — a plain removal from
// store.accounts would just get unioned back in on the next sync merge (see
// game/merge.js), since merge only ever adds/updates by id and never drops
// one side's records. Actually removing one for good means setting
// `deleted: true` (+ a fresh updatedAt) on the account and everything it
// owns and letting that sync out — for now that's done by hand against the
// D1 store directly. `!a.deleted` filtering below still applies so a
// manually-deleted account stays gone once synced.
